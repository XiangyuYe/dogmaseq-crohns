...
ct_rs <- c("CD4T", "CD4_CD103_TRM_Th17")
message(glue("{ct_rs[1]} to {ct_rs[2]}."))
celltype_id_rs <- c("ann_level4_final", "ann_level5_final")

#
ct_level <- readRDS("03_output/03_clustering/ct_levels_refine.rds")
all_ct_s <- ct_level[[celltype_id_rs[1]]]
senders_oi <- all_ct_s[grep("Treg", all_ct_s)]
all_ct_r <- ct_level[[celltype_id_rs[2]]]
receivers_oi <- all_ct_r[grep("CD4_CD103_TRM_Th17", all_ct_r)]
##
de_method <- "DESeq2"
message(de_method)
...
lapply(c("TI", "Colon"), function(type){
  ...
  lapply(c("II_vs_NN", "NU_vs_NN"), function(contrast_name){
    ...
    ## 1. load data
    meta_path <- glue("{data_path}{type}/all/metadata.tsv")
    tenX_path <- glue("{data_path}{type}/all/RNA/")
    sce_obj <- create.from.tenX(tenX_path = tenX_path,
                                ...
                                batch = batch)
    ## 2. set DE info
    de_path <- glue("03_output/04_Diff/DEG/{type}/{ct_rs}/{de_method}/")
    if (de_method == "MAST") {
      defile <- glue("{de_path}/lmm_{celltype_id_rs}_{contrast_name}.rds") %>% as.character()
    } else {
      defile <- glue("{de_path}/pseudo_bulk_{celltype_id_rs}_{contrast_name}.rds") %>% as.character()
    }
    celltype_de_fw <- lapply(defile, function(x){
    ...
    system(glue("mkdir -p {out_path}{type}/{ct_rs[1]}/{de_method}/{celltype_id}/"))
    saveRDS(multinichenet_output, glue("{out_path}{type}/{ct_rs[1]}/{de_method}/{celltype_id}/multinichenet_output_{contrast_name}_{ct_rs[2]}.rds"))
    #
    return(glue("{out_path}{type}/{ct_rs[1]}/{de_method}/{celltype_id}/multinichenet_output_{contrast_name}_{ct_rs[2]}.rds"))
  })
