...
sc_meta_b <- readRDS(glue("{data_path}{ct}/WNN_ADT_RNA/sc_meta.rds"))
sc_meta_b$Age <- scale(sc_meta_b$Age, center = T, scale = T) %>% as.numeric()
#
scATAC_obj <- readRDS("03_output/03_clustering/recall_comb_ann_level2_refine_scATAC_obj.rds")
scATAC_obj <- subset(scATAC_obj, cells = rownames(sc_meta_b))
DefaultAssay(scATAC_obj)<-"peaks"
##
sc_meta <- sc_meta_b[, c(block_col, sample_col, ann_col, 
