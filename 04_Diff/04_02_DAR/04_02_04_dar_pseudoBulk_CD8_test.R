...
sc_meta_cd8 <- readRDS(glue("{data_path}{ct}/WNN_ADT_RNA/sc_meta.rds"))
sc_meta_cd8$Age <- scale(sc_meta_cd8$Age, center = T, scale = T) %>% as.numeric()
#
scATAC_obj <- readRDS("03_output/03_clustering/recall_comb_ann_level2_refine_scATAC_obj.rds")
scATAC_obj <- subset(scATAC_obj, cells = rownames(sc_meta_cd8))
DefaultAssay(scATAC_obj)<-"peaks"
##
sc_meta <- sc_meta_cd8[, c(block_col, sample_col, ann_col, 
