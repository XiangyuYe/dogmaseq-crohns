...
##### 1. export SCENIC+ object #####
ctx <- "B"
data_path <- glue("03_output/03_clustering/{ctx}/")
scenic_path <- glue("03_output/07_SCENIC/{ctx}/")

sc_obj <- readRDS(glue("{data_path}WNN_ADT_RNA/scWNN_obj.rds"))
cutoff_q <- "q70"
...
region_set_path <- "03_output/07_SCENIC/B/outs/region_sets/"
#
cutoff_q <- "q70"
# scATAC_obj <- readRDS(glue("{data_path}scATAC_obj_test_{cutoff_q}.rds"))
...
dap_level4_by3 <- SplitObject(scATAC_obj, split.by = "ann_level3_final") %>%
  lapply(., function(scATAC_objx){
    ...
    if (length(unique(scATAC_objx$ann_level4_final)) > 1) {
      dap_level4_by3x <- FindAllMarkers(scATAC_objx, only.pos = T)
    } else {
      dap_level4_by3x <- NULL
    }
    return(dap_level4_by3x)
  }) %>% Reduce("rbind", .)
saveRDS(dap_level4_by3, file = glue("{data_path}dap_level4_by3.rds"))

##### 3. output as region set for SCENIC+ #####
