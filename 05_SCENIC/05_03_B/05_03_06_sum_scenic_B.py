...
## load data
out_path = '03_output/07_SCENIC/B/scplus_pipeline/Snakemake/'
...
rss = regulon_specificity_scores(
    ...
    variable = "scRNA_counts:ann_level3_final",
    modalities = ["direct_gene_based_AUC", "extended_gene_based_AUC",
    "direct_region_based_AUC", "extended_region_based_AUC"],
    selected_regulons = e_regulon_name_trans["signature"].to_list()
)
rss.to_csv(f"{out_path}/rss_B_ann_level3_final.txt", header=True, index=True, sep='\t', mode='w')

rssx = plot_rss(
    data_matrix = rss,
    top_n = 5,
    num_columns = 3,
    save = f"{out_path}/rss_B_ann_level3_final.pdf"
)
heatx = heatmap_dotplot(
    scplus_mudata = scplus_mdata,
    color_modality = "direct_gene_based_AUC",
    size_modality = "direct_region_based_AUC",
    group_variable = "scRNA_counts:ann_level3_final",
...
heatx.save(f"{out_path}/heat_B_ann_level3_final.pdf", format="pdf")

##
size_matrix = scplus_mdata["direct_region_based_AUC"].to_df()
color_matrix = scplus_mdata["direct_gene_based_AUC"].to_df()
group_by = scplus_mdata.obs["scRNA_counts:ann_level3_final"].tolist()
size_features, color_features, feature_names = scplus_mdata.uns["e_regulon_filter"][["Region_signature_name", "Gene_signature_name", "eRegulon_name"]] \
...
plotting_df = generate_dotplot_df(
        ...
        group_name = "scRNA_counts:ann_level3_final",
        size_name = "direct_region_based_AUC",
        color_name = "direct_gene_based_AUC",
        feature_name = "eRegulon_name"
)
plotting_df.to_csv(f"{out_path}/heatdot_B_ann_level3_final.txt", header=True, index=True, sep='\t', mode='w')
...
rss.to_csv(f"{out_path}/rss_B_ann_level4_final.txt", header=True, index=True, sep='\t', mode='w')

rssx = plot_rss(
    data_matrix = rss,
    top_n = 5,
    num_columns = 3,
    save = f"{out_path}/rss_B_ann_level4_final.pdf"
)
heatx = heatmap_dotplot(
    scplus_mudata = scplus_mdata,
    color_modality = "direct_gene_based_AUC",
...
heatx.save(f"{out_path}/heat_B_ann_level4_final.pdf", format="pdf")

##
size_matrix = scplus_mdata["direct_region_based_AUC"].to_df()
color_matrix = scplus_mdata["direct_gene_based_AUC"].to_df()
group_by = scplus_mdata.obs["scRNA_counts:ann_level4_final"].tolist()
size_features, color_features, feature_names = scplus_mdata.uns["e_regulon_filter"][["Region_signature_name", "Gene_signature_name", "eRegulon_name"]] \
    .drop_duplicates().values.T
...
plotting_df.to_csv(f"{out_path}/heatdot_B_ann_level4_final.txt", header=True, index=True, sep='\t', mode='w')
